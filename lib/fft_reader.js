  ...
  run(real, imag){
    ...
    for(let size = 2;size <= n;size *= 2){// logn
      ...
      for(let i = 0;i < n;i += size){// n / logn
        for(let j = i, k = 0; j < i + halfsize; j++, k+= tablestop){ //
    				let l = j + halfsize;
...
class Zombie{
  constructor(n){
    ...
    this.areal = new Array(m);
    this.aimag = new Array(m);

    this.breal = new Array(m);
    this.bimag = new Array(m);

    this.creal = new Array(m);
    this.cimag = new Array(m);
  }
