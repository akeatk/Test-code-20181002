...
//wolfram
/* 2 slope formula (input and prev slope)
Solve[-2 x α Sin[(α β)/2] Sin[(α β)/2 + ArcCos[γ/(x α)]] == γ - λ, x]
*/

/* 1 slope and slope diff formula (input and diff)
Solve[-2 x α Sin[(α β)/2] Sin[(α β)/2 + ArcCos[γ/(x α)]] == σ, x]
*/

/* pre divided 1slope and slope diff formula (input and diff/constants)
Solve[x Sin[(α β)/2 + ArcCos[γ/(x α)]] == ε, x]
*/
//code for pre divided
if(false){
  //constructor code
  this.p = 0;//previous slope
  this.w;//wavelength const
  this.c;//1/44100

  const trig_const = this.w * this.c / 2;
  const secant = Math.sec(trig_const);
  const tangent = Math.tan(trig_const);

  this.const0 = 2 * this.w * Math.sin(-trig_const);
  this.const1 = Math.pow(this.w, 2) * Math.pow(secant, 2);
  this.const2 = Math.pow(tangent, 2);
  this.const3 = 2 * this.w * tangent * secant;

  //code in function
  // func(slope)
  const sigma = (slope - this.p) / this.const0;

  const amplitude = Math.sqrt(
    this.const1 * sigma * sigma +
