...
if(true){
  ...
  console.log('comfort time = ',comfort * comfort);

  let arr = [];
  //write a function to find a bunch of combos and find
  //the one with lowest avg and lowest max step diff
  let brackets = [9,10,11,13,15,16];

  let time = 0;
  for(let i = 0;i < brackets.length;i++){
    time += Math.round(44100 / brackets[i] * 2) * Math.round(44100 / brackets[i] * 2);
  }
  console.log('multi-time   = ',time);

  for(let i = 0;i < brackets.length;i++){
    const arr_size = Math.round(44100 / brackets[i]);
    const step_size = 44100 / arr_size;
  ...
  console.log('max = ',max);
  console.log('avg below 200 = ',avg/c);
