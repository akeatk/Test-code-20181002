...
//tests for fft combos with high density early and/OR good uniformity overall
if(false){
...
if(true){
  const len = 128;
  const turkey = new FFTReader(len, 1);
  ...
  for(let i = 0;i < len;i++){
    r1[i] = 10 * Math.sin(5+Math.PI*2*i / (len / 15)) + 10 * Math.sin(3+Math.PI*2*i / (len / 5.01));
    i1[i] = 0;
  }
  ...
  ctx.fillStyle='#f00';
  for(let i = 0;i < len;i++){
    ctx.fillRect(i,0,1,r1[i]);
    ctx.fillRect(i,canvas.height - r2[i],1,r2[i]);
  }
